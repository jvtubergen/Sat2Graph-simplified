...
def graphDensifyWithClass(node_neighbor, node_class, density = 0.00020):
	visited = []
	...
	for node, node_nei in node_neighbor.iteritems():
		if len(node_nei) == 1 or len(node_nei) > 2:
			...
			for next_node in node_nei:
				...
				last_loc = node_list[0]
				if last_loc not in new_node_class:
					new_node_class[last_loc] = node_class[node_list[0]]
				if new_node_class[last_loc] > node_class[node_list[0]]:
					new_node_class[last_loc] = node_class[node_list[0]]
				...
				for i in range(interpolate_N):
					...
					for j in range(len(node_list)-1):
						if pd[j] <= int_d and pd[j+1] > int_d:
							...
							cid = max(node_class[node_list[j]],node_class[node_list[j+1]])

							if loc not in new_node_class:
								new_node_class[loc] = cid 
							if new_node_class[loc] > cid:
								new_node_class[loc] = cid
				...
				cid = new_node_class[last_loc]
				if node_list[-1] not in new_node_class:
					new_node_class[node_list[-1]] = cid 
				if new_node_class[node_list[-1]] > cid:
					new_node_class[node_list[-1]] = cid

	return new_node_neighbor, new_node_class
...
def graph2RegionCoordinateWithClass(node_neighbor, node_class, region, tile_size = 2048):
	new_node_neighbor = {}
	new_node_class = {}
	...
	for node, nei in node_neighbor.iteritems():
		loc0 = node 
		for loc1 in nei:
			x0 = (loc0[1] - region[1])/(region[3]-region[1])*tile_size
			y0 = (region[2]-loc0[0])/(region[2]-region[0])*tile_size
			x1 = (loc1[1] - region[1])/(region[3]-region[1])*tile_size
			y1 = (region[2]-loc1[0])/(region[2]-region[0])*tile_size

			n1key = (y0,x0)
			n2key = (y1,x1)

			if n1key not in new_node_class:
				new_node_class[n1key] = node_class[node]
...
def graphVis2048Segmentation(node_neighbor, region, filename, size=2048, width = 2):
	...
	for node, nei in node_neighbor.iteritems():
		...
		for loc1 in nei:
			...
			cv2.line(img, (x0,y0), (x1,y1), (255),width)
