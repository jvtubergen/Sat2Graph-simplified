...
import sys 
from decoder import DecodeAndVis, findClearKeypoints, graph_refine, _vis
...
def infer(sat):
    ...
    if tf_output is None:
        if int(sys.argv[1]) == 8009:
            pbfile = "../models/unet_frozen.pb"
        elif int(sys.argv[1]) == 8010:
            pbfile = "../models/deeproadmapper_frozen.pb"
        elif int(sys.argv[1]) == 8011:
            pbfile = "../models/jointorientation_frozen.pb"
    ...
    out = sess.run(tf_output, feed_dict={tf_inputsat: sat, tf_istraining: True})
    return out[0]
...
class S(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        weights = np.ones((image_size,image_size, 2)) * 0.001 
        ...
        mask = np.zeros((tile_size, tile_size, 2)) + 0.00001
        output = np.zeros((tile_size, tile_size, 2))
        ...
        segimg = (output[:,:,0]*255.0).astype(np.uint8)
        cv2.imwrite(output_file+"seg.png", segimg)

        cmd = "python seg2graph.py %s %s %s" % (output_file + "seg.png", str(int(0.15*255)), output_file+"rawgraph.p")
        Popen(cmd, shell=True).wait()
        graph_raw = pickle.load(open(output_file+"rawgraph.p"))
        graph = graph_refine(graph_raw, isolated_thr=200, spurs_thr=50)
        graph = simpilfyGraph(graph)

        _vis(graph, output_file+"refinegraph.png", size=tile_size)
        lines = []
        points = []
