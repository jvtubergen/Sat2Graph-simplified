...
def DecodeAndVis(imagegraph, filename, max_degree=6, thr=0.5, edge_thr = 0.5, snap=False, kp_limit = 500, drop=True, use_graph_refine=True, testing=False, spacenet = False, angledistance_weight = 100, snap_dist = 15, fast=False, w = 0, h = 0):
	...
	if np.shape(imagegraph)[2] == 2+4*max_degree + 5:
		# class
		classimg = (255.0 * imagegraph[:,:,2+4*max_degree:2+4*max_degree+3]).astype(np.uint8)
		cv2.imwrite(filename+"_roadtype.png",classimg)

		# road seg
		segimg = (255.0 * imagegraph[:,:,2+4*max_degree+3]).astype(np.uint8)
		cv2.imwrite(filename+"_roadseg.png",segimg)
	else:
		# road seg
		segimg = (255.0 * imagegraph[:,:,2+4*max_degree]).astype(np.uint8)
		cv2.imwrite(filename+"_roadseg.png",segimg)
