...
def infer(sat, road):
	...
	if tf_output is None:
		if int(sys.argv[1]) == 8008:
			pbfile = "../models/globalv2.pb"
		else:
			pbfile = "../models/usv2.pb"
		...
		tf_inputsat = tf.get_default_graph().get_tensor_by_name('import/inputsat:0')
		tf_inputroad = tf.get_default_graph().get_tensor_by_name('import/inputroad:0')
...
class S(BaseHTTPRequestHandler):
	...
	def do_POST(self):
		...
		weights = np.ones((image_size,image_size, 2+4*6 + 5)) * 0.001 
		...
		mask = np.zeros((tile_size, tile_size, 2+4*6 + 5)) + 0.00001
		output = np.zeros((tile_size, tile_size, 2+4*6 + 5))
		...
		# resample it 
		#input_keypoints[:,:,0] = findClearKeypoints(output[:,:,0], v_thr)
		...
		if "nPhase" in data:
			for phase in range(data["nPhase"]):
				mask *= 0.0 
				output *= 0.0
				for i in range(0,len(coordinates),batchsize):
					...
					for bt in range(N):
						x,y = coordinates[i+bt][0], coordinates[i+bt][1]
						sat_batch[bt,:,:,:] = sat_img[0,x*SAT_SCALE:x*SAT_SCALE+image_size*SAT_SCALE, y*SAT_SCALE:y*SAT_SCALE+image_size*SAT_SCALE,:] 
						road_batch[bt,:,:,:] = input_keypoints[x:x+image_size, y:y+image_size,:] 
				...
				print("GPU time (pass %d):"% (phase+2), time() - t0)
				t0 = time()

				output = np.divide(output, mask)
				graph = DecodeAndVis(output, output_file+"pass%d" % (phase+2) , thr=v_thr, edge_thr = e_thr, angledistance_weight=snap_w, snap_dist = snap_dist, snap=True, imagesize = tile_size, fast=fast)
				...
				print("Decode time (pass %d):" % (phase+2), time() - t0)

		t0 = time()
		graph = simpilfyGraph(graph)
		graphcolor = colorGraph(graph, output[:,:,2+4*6:2+4*6+3])

		# run mrf? 
		try:
			pickle.dump(graph, open("tmpgraph.p", "wb"), protocol=2)
			pickle.dump(graphcolor, open("tmpclass.p", "wb"), protocol=2)
			
			Popen("cd mrf; python mrf.py ../tmpgraph.p ../tmpclass.p ../tmpnewclass.p", shell=True).wait()

			#graphcolor = pickle.load(open("tmpnewclass.p","rb"), encoding="latin1") # python3
			graphcolor = pickle.load(open("tmpnewclass.p","r")) # python2
		except:
			print("mrf failed")
